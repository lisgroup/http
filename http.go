...
// Get 请求方式
func (c *Client) Get(url string) (result string, err error) {
	result, err = c.Request("GET", url)
	return
}

// Post 请求方式
func (c *Client) Post(url string) (result string, err error) {
	return c.Request("POST", url)
}
...
// cli 可选参数
func (c *Client) Request(rawUrl, method string) (result string, err error) {
	// 默认超时 20 秒
	if c.Timeout <= 0 {
		c.Timeout = 20
	}
